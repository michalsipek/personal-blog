...
<ul class="pager ">
	<c:if test="${page > 0}">
		<li class="previous"><a
			href='<spring:url value="/archive?date=${date}&page=${page-1}"></spring:url>'>Předchozí</a></li>
	</c:if>
	<c:if test="${page < sizeArticles / limitResultsPerPage-1}">
		<li class="next"><a
			href='<spring:url value="/archive?date=${date}&page=${page+1}"></spring:url>'>Další</a></li>
	</c:if>
</ul>
